# -*- coding: utf-8 -*-
"""
Created on Tue Sep 27 10:11:06 2022
...
#fething all data
path=pd.DataFrame(glob('D:/algo/NIfty_data/options sample data/sample_data_Jan2019/*'),columns=['location'])
...
trade_log=pd.DataFrame(columns=['Entry_date_time', 'Future_price', 'ATM','SL%', 'Days_to_expiry',
...
for index, row in path.iterrows():
    try:
        ...
        for i in range(9,15):
            entry_datetime=datetime.datetime.combine(data['date'][0],datetime.time(i,20))
            exit_dateitme=datetime.datetime.combine(data['date'][0],datetime.time(15,10))
            ...
            # CE data & PE data
            nearest_expiry=data['date'][0]+relativedelta(weekday=TH)
...
BTdata = trade_log
BTdata.to_csv(r'D:\\algo\\NIfty_data\\optBT.csv',index=None)
